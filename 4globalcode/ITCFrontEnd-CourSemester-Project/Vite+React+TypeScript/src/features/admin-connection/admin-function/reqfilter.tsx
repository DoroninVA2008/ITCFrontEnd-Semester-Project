...
const STATUS_BUTTONS = [
  { label: 'Все',          value: '' },
  { label: 'Новые',        value: 'Новая' },
  { label: 'На проверке',  value: 'На проверке' },
  { label: 'Отклонено',    value: 'Отклонена' },
  { label: 'Опубликовано', value: 'Опубликована' },
]
...
export const ReqFilter: React.FC<ReqFilterProps> = ({
  total,
  activeStatus,
  statusCounts,
  onStatusChange,
  ...
  return (
    <div className="admin-filters">
      {STATUS_BUTTONS.map(({ label, value }) => {
        const count = value === '' ? total : (statusCounts[value] ?? 0)
        return (
          <button
            key={value}
            className={activeStatus === value ? 'active' : undefined}
      ...
      <span className="search">
        ...
        <input
          type="search"
          placeholder="Поиск по названию, ID или отправителю"
